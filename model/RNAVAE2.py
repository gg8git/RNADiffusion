...
class Encoder(nn.Module):
    ...
    def forward(self, tokens: Tensor):
        ...
        hidden = self.blocks(emb)[:, : self.config.n_bn]
...
class Decoder(nn.Module):
    def __init__(self, config: Config):
        ...
        fbnd = config.zdim * config.n_bn
        self.dec_neck = nn.Sequential(
            nn.Flatten(1),
            nn.Linear(fbnd, fbnd),
            nn.Unflatten(1, (config.n_bn, config.zdim)),
            nn.Linear(config.zdim, config.d_model),
            nn.Tanh(),
            nn.Linear(config.d_model, config.d_model),
        )
...
def compute_stats(
    ...
    nll = cross_entropy(llog, ltok).mean()
