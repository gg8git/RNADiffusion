...
from datamodules.diffusion_datamodule import DiffusionDataModule, LatentDatasetDescriptors
from model import GaussianDiffusion1D, GPModelDKL, KarrasUnet1D
...
from utils.guacamol_utils import smile_to_guacamole_score, smiles_to_desired_scores
...
def normal_analytical_cond_fn(mean=0.0, sigma=0.001):
    ...
    def cond_fn(z, t, **guidance_kwargs):
        z = z.to(device)
        var = sigma**2
        log_prob = -((z - mean) ** 2).sum() / (2 * var) - z.numel() * torch.log(
            torch.sqrt(torch.tensor(2 * np.pi, device=z.device)) * sigma
        )
        return log_prob
...
def get_cond_fn(
    log_prob_fn, guidance_strength: float = 1.0, latent_dim: int = 128, clip_grad=False, clip_grad_max=10.0, debug=False
):
    """
...
def distribution_comparison(z, z_other, alpha=0.05, do_print=False):
    ...
    if do_print:
        print(
            f"Sample of shape: {z.shape} is {'' if is_different else 'not '}different from other with p={p_value:.4f}\n"
        )
    ...
    for method in ["ddim_haydn", "ddim_new_mean_cond", "ddim_no_cond"]:
        ...
        if method == "ddpm":
            latents = diffusion.ddpm_sample(
                ...
                class_labels=None,
                cond_fn=lambda x, t: cond_fn_grad_normal_dist(x.transpose(1, 2), t).transpose(1, 2),
            )
        elif method == "ddim":
            shape = (batch_size, diffusion.channels, diffusion.seq_length)
            latents = diffusion.ddim_sample(
                shape, class_labels=None, cond_fn=lambda x, t: cond_fn_normal_dist(x.transpose(1, 2), t)
            )
            ...
            latents = diffusion.ddim_sample(
                shape,
                class_labels=None,
                cond_fn=lambda x, t: cond_fn_normal_dist(x.transpose(1, 2), t),
                grad_scale=10.0,
                eta=0.1,
            )
            latents = latents.transpose(1, 2)
            ...
            shape = (batch_size, diffusion.channels, diffusion.seq_length)
            latents = diffusion.ddim_sample_haydn(
                shape,
                cond_fn=lambda x, t: cond_fn_grad_normal_dist(x.transpose(1, 2), t).transpose(1, 2),
                guidance_scale=1.0,
            )
            ...
            print("Method does not exist, defaulting to no conditioning.")
            shape = (batch_size, diffusion.channels, diffusion.seq_length)
        ...
        for i, latent in enumerate(latents):
            ...
            print(
                f"Sample {i + 1}: {smiles} | Valid: {is_valid_molecule(smiles)}, Mean Value: {latent.mean():.3f}, STD Deviation: {latent.std():.5f}"
            )
...
def validate_with_gp(diffusion, batch_size=64):
    ...
    surrogate_model = GPModelDKL(
        batch.reshape(data_batch_size, -1), likelihood=gpytorch.likelihoods.GaussianLikelihood().cuda()
    ).cuda()
    ...
    for method in ["ddim_haydn", "ddim_new_mean_cond", "ddim_no_cond", "optimize_acqf"]:
        ...
        if method == "ddpm_mean_cond":
            ...
            shape = (batch_size, diffusion.channels, diffusion.seq_length)
            latents = diffusion.ddim_sample(
                shape, class_labels=None, cond_fn=cond_fn_log_qei, bounds=bounds, grad_scale=10.0, eta=0.1
            )
            latents = latents.transpose(1, 2)
        elif method == "ddim_new_score_cond":
            shape = (batch_size, diffusion.channels, diffusion.seq_length)
            latents = diffusion.ddim_sample(
                shape, class_labels=None, cond_fn=cond_fn_log_qei, bounds=bounds, score_cond=True
            )
            latents = latents.transpose(1, 2)
            ...
            grad_qei_haydn = get_cond_fn(
                log_prob_fn_ei, clip_grad=False, latent_dim=(diffusion.channels * diffusion.seq_length)
            )
            shape = (batch_size, diffusion.channels, diffusion.seq_length)
            latents = diffusion.ddim_sample_haydn(
                shape,
                cond_fn=lambda x, t: grad_qei_haydn(x.transpose(1, 2), t)
                .reshape(-1, diffusion.seq_length, diffusion.channels)
                .transpose(1, 2),
                guidance_scale=8.0,
            )
            latents = latents.transpose(1, 2)
        ...
        for i, selfie in enumerate(selfies):
            ...
            if score:
                print(
                    f"Sample {i + 1}: {smiles} | Valid: {is_valid_molecule(smiles)}, Score: {score:.2f}, Prev Best Score: {best_f:.2f}, Within Bounds: {within_bounds_mask[i]}"
                )
                all_scores.append(score)
            else:
                print(
                    f"Sample {i + 1}: {smiles} | Valid: {is_valid_molecule(smiles)}, Within Bounds: {within_bounds_mask[i]}, error with score"
                )
                errors += 1
        ...
        summary.append(
            {
                ...
                "log qei": qEI_score.detach().cpu().item(),
                "score errors": errors,
                "bounds errors": (~within_bounds_mask).sum().item(),
            }
        )
    print(summary)
            ...
            for batch_size in batch_sizes:
                ...
                while log_qei_score == "N/A" and num_restarts < 5:
                    try:
                        shape = (batch_size, diffusion.channels, diffusion.seq_length)
                        latents = diffusion.ddim_sample_haydn(
                            shape, sampling_timesteps=50, cond_fn=cond_fn_ei_reshaped, guidance_scale=25.0
                        )
                        latents = latents.transpose(1, 2)  # with transpose
                        log_qei_score = log_qEI(latents.reshape(latents.shape[0], -1)).detach().cpu().item()
                    except Exception:
                ...
                while log_qei_score == "N/A" and num_restarts < 5:
                    try:
                        shape = (batch_size, diffusion.channels, diffusion.seq_length)
                        latents = diffusion.dpmpp2m_sample(
                            shape, sampling_timesteps=50, cond_fn=cond_fn_ei_reshaped, guidance_scale=25.0
                        )
                        latents = latents.transpose(1, 2)  # with transpose
                        log_qei_score = log_qEI(latents.reshape(latents.shape[0], -1)).detach().cpu().item()
                ...
                while log_qei_score == "N/A" and num_restarts < 5:
                    try:
                        shape = (batch_size, diffusion.channels, diffusion.seq_length)
                        latents = diffusion.dpmpp2msde_sample(
                            shape, sampling_timesteps=50, cond_fn=cond_fn_ei_reshaped, guidance_scale=25.0
                        )
                        latents = latents.transpose(1, 2)  # with transpose
                        log_qei_score = log_qEI(latents.reshape(latents.shape[0], -1)).detach().cpu().item()
                ...
                while log_qei_score == "N/A" and num_restarts < 5:
                    try:
                        latents, _ = optimize_acqf(
                            log_qEI, bounds=bounds, q=batch_size, num_restarts=10, raw_samples=1024
                        )
                        latents = latents.reshape(-1, diffusion.seq_length, diffusion.channels)
                        log_qei_score = log_qEI(latents.reshape(latents.shape[0], -1)).detach().cpu().item()
                    except Exception:
