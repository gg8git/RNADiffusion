...
class VAEModule(pl.LightningModule):
    def __init__(self, model):
        ...
        dec_params = (
            count_parameters(self.model.decoder)
            + count_parameters(self.model.dec_neck)
            + count_parameters(self.model.dec_tok_deproj)
        )
    ...
    def configure_optimizers(self):
        ...
        return dict(optimizer=opt, lr_scheduler=dict(scheduler=lr_sched, interval="step", frequency=1))
    ...
    def sample(self, z: torch.Tensor, argmax=True, max_len=256):
        ...
        tokens = torch.full(
            (z.shape[0], 1), fill_value=self.model.start_tok, device=self.model.device, dtype=torch.long
        )
    ...
    def forward_selfies(self, selfies: str | list[str]):
    ...
    def latent_to_selfies_batch(self, z: torch.Tensor, argmax=True, max_len=256):
        ...
        tokens = self.sample(z, argmax=argmax, max_len=max_len)
